...
export class OAuthAuthProvider implements core.AuthProvider {
    ...
    public static canCreate(options: OAuthAuthProvider.Options): boolean {
        return (
            (options.clientId != null || process.env?.KARD_CLIENT_ID != null) &&
            (options.clientSecret != null || process.env?.KARD_CLIENT_SECRET != null)
        );
    }
    ...
    private async refresh(_arg?: { endpointMetadata?: core.EndpointMetadata }): Promise<string> {
        this._refreshPromise = (async () => {
            try {
                const clientId = (await core.Supplier.get(this._clientId)) ?? process.env?.KARD_CLIENT_ID;
                if (clientId == null) {
                    throw new errors.KardApiError({
                        message:
                            "clientId is required; either pass it as an argument or set the KARD_CLIENT_ID environment variable",
                    });
                }

                const clientSecret = (await core.Supplier.get(this._clientSecret)) ?? process.env?.KARD_CLIENT_SECRET;
                if (clientSecret == null) {
                    throw new errors.KardApiError({
                        message:
                            "clientSecret is required; either pass it as an argument or set the KARD_CLIENT_SECRET environment variable",
                    });
                }
